...
/*
 ...
 2. Listen to global drag state changes with `DragLayer.listen`. These are all available on `IDragLayerMonitor`:
...
export interface DragLayer<Item = any>
   ...
   *  the client coordinates of the dragged item. Read the {@link IDragLayerMonitor}
   *  docs to see all the different possibile coordinates you might subscribe
   *  to.
   */
  listen<O>(mapTo: (monitor: IDragLayerMonitor<Item>) => O): Observable<O>;
}
