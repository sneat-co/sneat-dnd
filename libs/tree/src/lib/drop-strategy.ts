...
export class DefaultDropStrategy<Item> implements IDropStrategy<Item> {
    ...
    public suggestDropPosition(
        monitor: IDropTargetMonitor<IDraggedTreeItem<Item>>,
        hovered: IDropHovered<Item>,
    ): IDropTargetPosition<Item> {
        const dragged = monitor.getItem();
        if (!dragged) {
            throw new Error('no dragged node to suggest position for');
        }
        ...
        if (!hovered.node.parent) {
            throw new Error('hovered node has no parent');
        }
    ...
    suggestTripleWay(dragged: IDraggedTreeItem<Item>, hovered: IDropHovered<Item>, clientOffset: IOffset): IDropTargetPosition<Item> {
        ...
        if (!hoveredNode.parent) {
            throw new Error('somehow root node hovered (should be invisible)');
        }
        const draggedNode = hoveredNode.tree.state.node(draggedId);
        if (!draggedNode.parent) {
            throw new Error('somehow root node dragged (should be invisible)');
        }
        if (mouse < start + padding) {
            ...
            if (hovered.node.children?.some(child => child === draggedId)) {
                result = {parent: hoveredNode.parent, index: hovered.node.index + 1};
