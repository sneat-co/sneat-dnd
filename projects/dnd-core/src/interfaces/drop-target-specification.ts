import {IDropTargetMonitor} from './drop-target-monitor';
 ...
 * discussion of these type parameters at {@link IDragSourceSpec}.
 */
export interface IDropTargetSpec<Item extends {} = {},
   ...
   * */
  canDrop?(monitor: IDropTargetMonitor<Item, DropResult>): boolean;

  /* Called frequently while the mouse hovers over the owner drop target while
   * dragging a relevant item.
   * */
  hover?(monitor: IDropTargetMonitor<Item, DropResult>): void;
   ...
   * {@link IDropTargetMonitor#getDropResult}. This is useful in case you want the
   ...
   * already handled drop by checking {@link IDropTargetMonitor#didDrop}. Both this
   ...
   * */
  drop?(monitor: IDropTargetMonitor<Item, DropResult>): DropResult | void;
}
